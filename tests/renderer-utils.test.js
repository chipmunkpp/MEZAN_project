...
test("filterTransactions matches search and date range", () => {
  const txns = [
...
test("summarizeTransactions computes period totals in one pass", () => {
  const txns = [
...
test("buildSevenDayNet groups per day", () => {
  const txns = [
...
test("buildSixMonthTrend groups per month", () => {
  const txns = [
    { date: "2026-01-15", type: "income", amount: 300 },
  ...
  assert.deepEqual(months[3], { lbl: "01", inc: 300, exp: 0, net: 300 });
